...
const BuildingPage = () => {
        ...
        const fetchBuildings = async () => {
            ...
            try {
                ...
                const apiEndpoint = showArchived
                    ? "/api/building?status=Archived" // For archived
                ...
                if (showArchived) {
                    setArchivedData(data); // Set archived data
                    setFilteredData(data); // Set filtered data to archived
                } else {
                    setData(data); // Set active data
                    setFilteredData(data); // Set filtered data to active
                }
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            ...
            setData((prevData) =>
                prevData.filter(
                    (building) => !selectedRowKeys.includes(building.id)
                )
            );
