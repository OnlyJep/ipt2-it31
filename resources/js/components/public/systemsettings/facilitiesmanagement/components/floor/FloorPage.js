...
const FloorPage = () => {
    ...
    const fetchArchivedData = async () => {
        ...
        setError(null);
        try {
            const response = await axios.get(`/api/floor?status=Archived`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
    ...
    const handleDeleteSelected = async () => {
        const selectedFloors = data.filter((floor) =>
            selectedRowKeys.includes(floor.id)
        );
        ...
        try {
            ...
            const archivedThese = selectedFloors.map((floor) => ({
                ...floor,
                isArchived: true,
                deleted_at: now,
            }));
            const remainingFloors = data.filter(
                (floor) => !selectedRowKeys.includes(floor.id)
            );
            ...
            setSelectedRowKeys([]);
            message.success(
                `${selectedFloors.length} floor(s) archived successfully`
            );
        } catch (error) {
    ...
    const handleRestoreSelected = async () => {
        const selectedFloors = archivedData.filter((floor) =>
            selectedRowKeys.includes(floor.id)
        );
        if (selectedFloors.length === 0) return;
        ...
        try {
            ...
            const remainingArchivedFloors = archivedData.filter(
                (floor) => !selectedRowKeys.includes(floor.id)
            );
            ...
            setSelectedRowKeys([]);
            message.success(
                `${selectedFloors.length} floor(s) restored successfully`
            );
        } catch (error) {
    ...
    const handleRestoreFloor = async (id) => {
        try {
            ...
            const floorToRestore = archivedData.find(
                (floor) => floor.id === id
            );
            if (floorToRestore) {
                const updatedArchived = archivedData.filter(
                    (floor) => floor.id !== id
                );
    ...
    const handleCreateFloor = async (floorData) => {
        try {
            // **Duplicate Check**
            const duplicate = [...data, ...archivedData].some(
                (floor) =>
                    String(floor.floor_level).toLowerCase() ===
                    String(floorData.floor_level).toLowerCase()
            );
    ...
    const handleEditFloor = async (id, updatedData) => {
        try {
            ...
            const duplicate = [...data, ...archivedData].some(
                (floor) =>
                    String(floor.floor_level).toLowerCase() ===
                        String(updatedData.floor_level).toLowerCase() &&
                    floor.id !== id
            );
    ...
    const handlePrint = () => {
        ...
        printWindow.document.write(
            "<html><head><title>Floor Table</title></head><body>"
        );
        ...
        filteredData.forEach((floor) => {
            ...
            if (!showArchived) {
                printWindow.document.write(
                    `<td>${formatDate(floor.created_at)}</td>`
                );
                printWindow.document.write(
                    `<td>${formatDate(floor.updated_at)}</td>`
                );
            } else {
                printWindow.document.write(
                    `<td>${formatDate(floor.deleted_at)}</td>`
                );
            }
            printWindow.document.write("</tr>");
        });
